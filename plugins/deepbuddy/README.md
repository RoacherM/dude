...
- **工作区面板**：浏览器式压缩标签条（文件 tab 真实）；活动栏四视图（概览/文件/浏览器/Git）；停靠栏文件树为真实懒加载树（`deepbuddyFiles/*` 端点，会话围栏）；文件内容视图真实（文本/二进制/截断/拒绝态）；浏览器与概览与 Git 视图为演示数据。
- **设置**：主列页面，保留侧栏与停靠栏状态；Esc 或关闭按钮返回会话。
