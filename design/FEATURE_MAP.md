...
> 用途：把 DeepBuddy 自研的功能落到明确区域、数据源和状态 Owner。
> 它是现状清单，不是长期公共 API。
...
|---|---|---|---|---|
| Desktop Shell | 全窗 | DeepBuddy | Electron Window / 原生集成 | Electron Main |
...
| 窗口拖拽区域 | 官方元素空白处 | DeepBuddy | CSS `-webkit-app-region` | `ctx.effect`（样式表） |
| Hero 品牌标记 | 官方 Hero | DeepBuddy | `conversation.hero.brand.mark` 槽 | `ctx.effect`（slot 注册） |
| 正文字体 | 全窗 | DeepBuddy | `--dsw-font-family` 覆写 | 随样式表安装 |
...
DeepBuddy 客户端只有两个模块（见 `ARCHITECTURE.md` §3）：
...
以下功能曾经由 DeepBuddy 自己实现，现已整体删除，由官方 dockkit 右栏（自带 Files 页）
取代：
...
DeepBuddy 当前架构被认为跑通，当：

- 官方 Sidebar / Conversation / Rightbar 全部原生可用，DeepBuddy 未做任何包装；
- 插件卸载后官方界面完整可用，没有残留状态或死代码路径；
- 窗口拖拽区域覆盖 macOS 无边框窗口的可用性需求，且不挡任何官方控件点击；
- Hero 品牌标记正确渲染 DeepBuddy 版本，官方标题与徽标不受影响；
