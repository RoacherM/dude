...
| 密行高度 | 26px |
| 左列默认宽度 | 268px |
| 左列拖拽范围 | 232px – 窗口的 1/4，且受共享天花板约束：任一侧列的拖拽上限 = 窗口 − 内边距/缝 − 另一列 − 主列 460 保底。拖 A 不产生要 B 在放手时偿还的债 |
| 右列默认宽度 | 窗口的 30% |
| 右列拖拽范围 | 416px – 窗口的 1/2。下限是固定像素而非比例，宽屏不抬高最窄宽度；30% 只是打开时的默认宽 |
| 右列最小宽度 | 416px，与 1/2 上限冲突时（窄窗小区间）以 416px 优先；删除控件后应重新验证依据 |
| 主列保留宽度 | 460px |
...
自动收列阈值仍按窗口宽度判断，不因此调整。比例（30% / 1/2）一律按窗口宽度计算，
