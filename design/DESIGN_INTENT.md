...
DeepBuddy 不拥有布局。窗口的列结构、列的开关、列内容、响应式收列，全部由官方
`ui-layout` / `ui-sidebar` / `ui-conversation` / dockkit（官方右栏，自带 Files 页）
决定。DeepBuddy 在这套官方结构上只叠加两层：

```text
窗口拖拽区域   —— 让无边框窗口在 macOS 上可以用鼠标拖动、不挡官方控件的点击
Hero 品牌标记  —— 官方 Hero 的 conversation.hero.brand.mark 槽渲染 DeepBuddy 的鱼形标记
```

没有 Sidebar / Workbench / Inspector 三列职责划分，没有 DeepBuddy 自己的状态栏、
...
无边框窗口（macOS `hiddenInset`）需要显式声明哪些区域可以拖动整个窗口。DeepBuddy 的
样式表（`plugins/deepbuddy/src/client/ui/styles.ts`）在以下官方区域上加
...
是官方的鱼形 Logo（`FishLogo`，priority 0）。DeepBuddy 在同一个槽以 priority -1 注册
自己的版本（`DeepBuddyBrandMark`，`plugins/deepbuddy/src/client/dsh/adapter.ts`），
两者矢量路径与尺寸对齐（34×25，`FISH_LOGO_PATH`），单 occupant 槽只渲染较低优先级的
DeepBuddy 版本。官方的标题（“探索未至之境”）和预览徽标（“预览版”）不受影响，因为它们
...
DeepBuddy 不维护独立的颜色、圆角、间距或组件体系。界面的绝大部分视觉由官方主题
（`--dsw-*` 变量）决定，DeepBuddy 只覆写两处：

- **正文字体**：`--dsw-font-family` 改为 Archivo（Omnibus Type，OFL-1.1，latin 变量子集
  内嵌为 woff2），中文回退到 PingFang SC / 系统字体栈；代码字体
  `--ds-font-family-code` 保持系统等宽栈不变（`plugins/deepbuddy/src/client/ui/fonts.ts`）。
- **Hero 品牌标记**：见第 3 节。

没有 DeepBuddy 专属的 Token 前缀、KIT 组件库或暗色方案定义——这些随三列壳、Inspector
